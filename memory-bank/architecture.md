...
- `RSSRadarApp/Views/AppRootView.swift`：主根视图。首次使用时通过 `@AppStorage("hasCompletedOnboarding")` 展示 `OnboardingView`；完成 onboarding 后展示 `NavigationSplitView` app shell。侧边栏使用 `List(selection:)`、`ForEach` 和每行 `.tag(section)` 显式绑定 `RSSRadarSection`，保证 Today、Topics、Feeds、Processing、Settings 点击后更新 detail selection。当前 Today 和 Topics 仍是占位，Feeds、Processing、Settings 已接入真实页面与 ViewModel。Step 32 后 Processing 的 missing-key/AI-failure 状态可通过闭包让根视图切换到 Settings；根视图仍只承载 UI 组合，不直接访问 SQLite、AI Provider 或 RSS 抓取实现。
