...
- `Package.swift`：Swift Package 根 manifest，声明 macOS 14.0+、`RSSRadar` executable product、内部 library products、target 依赖关系和测试 target。当前外部包依赖包含 Step 4 所需的 GRDB.swift、Step 8 所需的 FeedKit、Step 12 所需的 SwiftSoup；`RSSRadarPersistence` 额外链接 Apple `Security` framework 以使用 macOS Keychain；`RSSRadarAI` 处理 `Prompts/` 资源目录；`RSSRadarExport` 依赖 Core 与 Processing 的 Topic Detail DTO 进行 Markdown 渲染、复制和文件导出。当前测试 target 包含 Core、Persistence、Feeds、AI、Processing、Export 各模块测试。后续依赖必须按实施计划逐步加入。
...
- `Packages/RSSRadarPersistence/Sources/RSSRadarPersistence/UserCorrectionRepository.swift`：`UserCorrection` Repository。支持纠错记录 save/upsert、按 ID 读取、全量列表、按 topic 查询和按 article 查询；只负责 `user_corrections` 表映射，不直接改 `topic_articles` 或 `topics`，具体纠错动作由 Processing 层 use case 事务编排。
...
- `Packages/RSSRadarProcessing/Sources/RSSRadarProcessing/ProcessingEngineExecutor.swift`：生产默认 job executor。当前真实执行能力覆盖 `fetch_feed`、显式注入 `ArticleAnalyzing` 后的 `analyze_article`、显式注入 `TopicAssigning` 后的 `assign_topics`、显式注入 `TopicBriefGenerating` 后的 `generate_topic_brief`；`parse_article` 与 `retry_failed_job` 仍保持 unsupported。该文件只做 job payload 解包与 use case 调用，不自行读取 Keychain 或构建网络 AI provider。
